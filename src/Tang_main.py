...
from sklearn.metrics import roc_auc_score, precision_recall_curve, roc_curve, mean_squared_error
...
def parse_args():
    ...
    parser.add_argument('--alpha', type=int, default=0.2,
                        help='alpha')
    parser.add_argument('--com_number', type=int, default=1351,
                        help='total_iter')
    parser.add_argument('--kin_number', type=int, default=188,
                        help='total_iter')
    return parser.parse_args()

# args,data_cK_new,Kin_pre,data_cPre,test0,test1,data_cK
def model_train(args, data_cK, Kin_pre, data_cPre, test0, test1, MD_matOrigin):
    ...
    Xseq1 = tf.sigmoid(tf.matmul(tf.matmul(aa, xs_b), W1))  # 图卷积神经网络
    Xseq2 = tf.sigmoid(tf.matmul(tf.matmul(aa, Xseq1), W2))  # 图卷积神经网络  0.9022;0.2068
    Xseq3 = tf.sigmoid(tf.matmul(tf.matmul(aa, Xseq2), W3))  # 图卷积神经网络   0.9001;0.2219
    Xseq = tf.sigmoid(tf.matmul(tf.matmul(aa, Xseq3), W4))  # 图卷积神经网络    0.8964;0.2061
    ...
    loss = tf.losses.mean_squared_error(Yture,Ypre)
...
if __name__ == '__main__':
    ...
    data_CompKin = read_data(data_path + '\Tang_Matrix_Compound_Kinase.txt')
    data_CompPre = read_data(data_path + '\Tang_Compound_feature1024.txt')
    data_KinSeq = read_data2(data_path + '\Tang_KinSeq.txt')
